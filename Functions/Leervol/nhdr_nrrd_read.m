...
% - raw, txt/text/ascii, hex, gz/gzip encodings
...
headerInfo = struct();
% default header:
headerInfo.content = mainFileName;        % default value which can be overriden if content field is set
headerInfo.data = [];
fidr = fopen( nrrdFileName, 'r' );
if (fidr == -1)
    error('nhdr_nrrd_read ABORT: %s does not exist.\n', nrrdFileName );
end
try 
    ...
    % Magic line
    cs  = fgetl(fidr);
    assert(numel(cs)>=4 && isequal(cs(1:4), 'NRRD'), 'Bad signature. First line should be magic line of type "NRRD000X" with X an integer between 1 and 5.'); % FIXME should throw an error if a bad integer is provided
    ...
    while ( ~feof(fidr) )
        ...
        elseif ( foundKeyword('KINDS:', cs ) )
            ...
            % FIXME: check that axis sizes match each kind according to standard
        elseif ( foundKeyword('SPACINGS:', cs ) ) 
                ind=find(cs==' ');
                spacings(1)=str2num(cs(ind(1):ind(2)));
                spacings(2)=str2num(cs(ind(2):ind(3)));
                spacings(3)=str2num(cs(ind(3):end));
                headerInfo.spacings=spacings;
        elseif ( foundKeyword('LABELS:', cs ) )    
             aux=cs(length('labels:  '):end);
             ind=find(aux~='"' & aux~=' '); 
             headerInfo.labels=aux(ind);
        elseif ( foundKeyword('SPACE:', cs ) )
            % Starts defining orientation (either space or space dimension, not
            % both)
            if isfield(headerInfo, 'spacedimension')
                fprintf('WARNING nhdr_nrrd_read %s: ignored field ''space'' because ''space dimension'' was specified before.\n', fopen(fidr));
            else
                headerInfo.space = strtrim( cs( length('SPACE:')+1:end ) );
                internal_spacedimension = nrrd_getSpaceDimensions(headerInfo.space);  % for internal use only
                assert(internal_spacedimension > 0, sprintf('nhdr_nrrd_read %s: unrecognized ''space'' descriptor ''%s''\n',fopen(fidr), headerInfo.space));
        ...
        elseif ( foundKeyword('SPACE DIMENSION:', cs) )
            ...
            if isfield(headerInfo, 'space')
                fprintf('WARNING nhdr_nrrd_read %s: ignored field ''space dimension'' because ''space'' was specified before.\n', fopen(fidr));
            else
                headerInfo.spacedimension = sscanf( cs( length('SPACE DIMENSION:')+1:end), '%i' );
                internal_spacedimension = headerInfo.spacedimension;  % for internal use only
                assert(numel(headerInfo.spacedimension)==1,...
                        sprintf('nhdr_nrrd_read %s: ''space dimension'' should be specified as one integer number. Found %d element(s) instead.\n',...
                                fopen(fidr), numel(headerInfo.spacedimension)));
                assert(headerInfo.spacedimension > 0, ...
                        sprintf('nhdr_nrrd_read %s: ''space dimension'' should be specified as a strictly positive integer (found %d).\n',...
                                fopen(fidr),headerInfo.spacedimension));
                define_orientation = 1;
            end
            
        elseif ( foundKeyword('SPACE DIRECTIONS:', cs ) )
            % Required if orientation defined but must come after space or space
            % dimension
            % space directions: <vector[0]> <vector[1]> ... <vector[dim-1]>
            
            assert(define_orientation==1, ...
                    sprintf('nhdr_nrrd_read %s: field ''space directions'' cannot be specified before ''space'' or ''space dimension''.\n',fopen(fidr)));
            % FIXME: ideally, check that the sum of elements including none and
            % (.,.,.) matches headerInfo.dimension, i.e. the total dimension.This
            % script currently gladly accepts absurd strings such as '1 0, 0 0 1 ( 0 none  0 ) 0 1,' 
            % instead of '(1,0,0) none none (0,1,0)  (0,0,1) none ' for instance. 
            % Maybe a problem ? Many people seem to mis-specify this
            % field so it may be a good idea to cut them some slack.
            space_dir_tmp = strtrim(cs(length('SPACE DIRECTIONS:')+1:end));     % remove leading and trailing spaces
            headerInfo.spacedirections = strsplit(space_dir_tmp);               % cell array of strings, ideally of the form {'(1,0,0)' '(0,2,0.1)' 'none'  '(0.1,0.1,1.1)'} if internal_spacedimension==3
            SD_data = strrep(space_dir_tmp, 'none', '' );                       % ignore "none" entries
            SD_data = extractNumbersWithout(SD_data, {'(',')',','} );           % detects numbers up to first non numerical entry
            assert(numel(SD_data) == (internal_spacedimension)^2,...
                   sprintf('nhdr_nrrd_read %s: expected ''space directions'' to specify a %d-by-%d matrix (%d total elements) in agreement with world space dimension. Found %d element(s) instead.\n',...
                            fopen(fidr),internal_spacedimension, internal_spacedimension, (internal_spacedimension)^2, numel(SD_data)) );
            headerInfo.spacedirections_matrix = reshape(SD_data(:), [internal_spacedimension, internal_spacedimension]);
            % correctness of field specification is checked below after 
            % whole header is parsed because we need to be sure that the 
            % dimension field was set
            
        elseif ( foundKeyword('SPACE UNITS:', cs ) )
            % Always optional, must come after space or space dimension
            
            assert(define_orientation==1,...
                    sprintf('nhdr_nrrd_read %s: field ''space units'' cannot be specified before ''space'' or ''space dimension''.\n', fopen(fidr)));
            ...
            assert(length(headerInfo.spaceunits) == internal_spacedimension,...
                    sprintf('nhdr_nrrd_read %s: expected ''space units'' to contain %d elements enclosed in double quotes to match the ''space'' or ''space dimension'' field but found the following %d element(s):\n%s\n',...
                            fopen(fidr), internal_spacedimension, length(headerInfo.spaceunits), sprintf('%s\t',headerInfo.spaceunits{:}) ));
            
        elseif ( foundKeyword('SPACE ORIGIN:', cs ) )
            % Always optional, must come after space or space dimension
            
            assert(define_orientation==1, sprintf('nhdr_nrrd_read %s: field ''space origin'' cannot be specified before ''space'' or ''space dimension''.\n', fopen(fidr)));
            
            iSO = extractNumbersWithout( cs(length('SPACE ORIGIN:')+1:end), {'(',')',','} );
            
            assert(numel(iSO) == internal_spacedimension,...
                    sprintf('nhdr_nrrd_read %s: expected ''space origin'' to specify a %d-element vector to match the ''space'' or ''space dimension'' field but found %d element(s).\n',...
        ...
        elseif ( foundKeyword('MEASUREMENT FRAME:', cs ) )
            ...
            assert(define_orientation==1,...
                    sprintf('nhdr_nrrd_read %s: field ''measurement frame'' cannot be specified before ''space'' or ''space dimension''.\n', fopen(fidr)));
            ...
            assert(numel(iMF) == (internal_spacedimension)^2,...
                sprintf('nhdr_nrrd_read %s: expected ''measurement frame'' to specify a %d-by-%d matrix (%d total elements) but found %d element(s).\n',...
                        fopen(fidr), internal_spacedimension, internal_spacedimension, (internal_spacedimension)^2, numel(iMF)));
        ...
        elseif ( foundKeyword('LINE SKIP:', cs) || foundKeyword('LINESKIP', cs) )
            ...
            assert(headerInfo.lineskip >= 0,...
                    sprintf('Field lineskip or line skip should be greater than or equal to zero, detected %d.\n', headerInfo.lineskip));
        elseif ( foundKeyword('BYTE SKIP:', cs) || foundKeyword('BYTESKIP:', cs) )
            ...
            assert(headerInfo.byteskip >= -1, sprintf('Field byteskip or byte skip can only take non-negative integer values or -1, detected %d.\n',headerInfo.byteskip));
    ...
    if isfield(headerInfo, 'byteskip') && headerInfo.byteskip == -1
        assert( strcmpi(headerInfo.encoding, 'raw'), sprintf('byte skip value of -1 is only valid with raw encoding. See definition of NRRD File Format for more information.\n'));
    end
    ...
    if define_orientation
        assert(isfield(headerInfo, 'spacedirections'), 'Missing field ''space directions'', required if either ''space'' or ''space dimension'' is set.');
        if length(headerInfo.spacedirections) ~= headerInfo.dimension
            fprintf('WARNING nhdr_nrrd_read %s:\n Unexpected format found for ''space directions'' specifier.\n Expected a combination of none and vectors delimited by parentheses such as (1.2,0.2,0.25), see definition of NRRD File Format for more information.\n', fopen(fidr));
        end
    end
    
    % TODO: add support for positive line skips 
    if isfield(headerInfo, 'lineskip') && headerInfo.lineskip > 0
        assert(isfield(headerInfo, 'byteskip') && headerInfo.byteskip == -1, sprintf('lineskip option is currently not supported and can only be set to zero unless raw encoding is used and byte skip is set to -1 (which cancels the effect of lineskip altogether).\n'));
    end  
    
    % TODO: add support for positive byte skips (see line skip)
    if isfield(headerInfo, 'byteskip') && ~strcmpi(headerInfo.encoding,'raw')
        assert( headerInfo.byteskip == 0, sprintf('byte skip option with non raw encoding is currently not supported and can only be set to zero.\n'));
    end
    if isfield(headerInfo, 'byteskip') && strcmpi(headerInfo.encoding, 'raw')
        assert( headerInfo.byteskip == -1, sprintf('non-negative byte skip values with raw encoding are currently not supported; byte skip can only be set to -1.\n'));
    end
...
if bReadData
    ...
    if isfield(headerInfo, 'datafiles')
        ...
        if ~isempty(subdim) && subdim~=headerInfo.dimension
            error('nhdr_nrrd_read (detached header): reading data from slices along axis other than the last (i.e. slowest) one, is currently not supported.\nLast argument [<subdim>] in ''data file'' or ''datafile'' field should be removed or set equal to %d, which is the detected ''dimension'' field value, for now.\n',...
        ...
        for i = 1:N_data_files
            ...
            elseif strcmpi(ext_data, '.nrrd')
                ...
                tmp_struct = nhdr_nrrd_read(fullfile(mainFpath, [fname_data, ext_data]), bRead_Detached_Data);   % recursive call
                % TODO: check the rest of the structure for
                % inconsistencies with metadata from header
                assert(N_data_files==1 || headerInfo.dimension == tmp_struct.dimension +1, sprintf(...
                    'nhdr_nrrd_read detached header %s: the number of dimensions in detached nrrd data file %d/%d (%s) should be one fewer than that of the header file.\nDetected %d instead of %d.\nDifferent datafile dimensions as specified by the nrrd standard are not supported as of now. \n',...
                    fopen(fidr), i, N_data_files, headerInfo.datafiles{i}, tmp_struct.dimension, headerInfo.dimension-1 ));
                ...
                if( fid_data < 1 )
                    error('While reading detached header file %s: detached data file number %d/%d (%s) could not be opened.\n', nrrdFileName, i, N_data_files, headerInfo.datafiles{i});
                end
                try
...
if length(field_string)>= 4 && strcmpi(field_string(1:4), 'LIST')
    ...
    elseif any(length(str_lst)==[0, 2, 3]) || length(str_lst)>5 
        ...
        error('error in ''data list'' (or ''data list'') field, in non ''LIST'' mode:\nexpected  ''<filename>'' or ''<format> <min> <max> <step> [<subdim>]'' but found instead ''%s'' (%d elements instead of 1, 4 or 5).\n',...
        ...
        assert(step~=0,sprintf('detached data files with names specified by sprintf()-like format:  step should be strictly positive or negative, not zero.\n'));
        if step < 0
            assert(id_min >= id_max, sprintf('detached data files with names specified by sprintf()-like format:  when step is <0, min should be larger than or equal to max, here we found min=%d < max=%d.\n',id_min, id_max));
        else
            assert(id_min <= id_max, sprintf('detached data files with names specified by sprintf()-like format: : when step is >0, min should be smaller than or equal to max, here we found min=%d > max=%d.\n',id_min, id_max));
        end
...
assert(Nelems == numel(data), sprintf('Error reading binary content of %s: detected %d elements instead of %d announced in header.\n',fopen(fidIn), numel(data), Nelems));
end
